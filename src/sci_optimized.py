"""SCI: Staged Conditional Integration - Optimized Version

Three orthogonal modules combined for superior performance:
- PCET: Class prediction (what class)
- SRGC: Sample uncertainty (how confident)
- SIED: Domain adaptation (source-target alignment)
"""
import os
...
def compute_uncertainty(X, mu_0, sigma_0, mu_1, sigma_1):
    """SRGC: Mahalanobis distance based uncertainty"""
...
def compute_domain_similarity(X_cal, y_cal, X_test, source_stats):
    """SIED: Domain similarity via likelihood ratio"""
...
print("="*70)
print("SCI: Staged Conditional Integration Framework")
print("="*70)
...
for seed in seeds:
    ...
    for held_out in Y_SUBJECTS:
        ...
        for n_cal in shot_settings:
            ...
            try:
                ...
                auroc_eeg = auroc_pcet = auroc_srgc = auroc_domain = 0.5
    ...
    df_partial.to_csv(f"{RESULTS_DIR}/sci_optimized_partial.csv", index=False)

df = pd.DataFrame(results)
df.to_csv(f"{RESULTS_DIR}/sci_optimized_results.csv", index=False)
...
for n_cal in shot_settings:
    ...
    for method in ['EEG_SVM', 'PCET', 'SRGC', 'SIED']:
        ...
        if len(method_df) > 0:
            ...
            summary_data.append({'shot': n_cal, 'method': method, 'accuracy': acc, 'std': std})

    print(f"  {'SCI_best':15s}: {best_sci_acc:.4f} ({best_sci_name})", flush=True)
    summary_data.append({'shot': n_cal, 'method': best_sci_name, 'accuracy': best_sci_acc, 'std': 0})

df_summary = pd.DataFrame(summary_data)
df_summary.to_csv(f"{RESULTS_DIR}/sci_summary.csv", index=False)
...
for n_cal in shot_settings:
    ...
    print(f"{n_cal}-shot: Best SCI={best_sci_acc:.4f} vs Best Single={best_single:.4f} (Δ={improvement:+.4f}) [{best_sci_name}]")
