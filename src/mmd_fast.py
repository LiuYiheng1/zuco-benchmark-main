"""
EEG MMD Pilot Experiments (Optimized - faster version)
"""
import os
import numpy as np
import pandas as pd
...
def mmd_loss_simple(z, sub_ids, device):
    """Simplified MMD loss: penalize variance across subject means"""
    unique_subjects = torch.unique(sub_ids)
    ...
    for subj in unique_subjects:
        mask = sub_ids == subj
        if mask.sum() > 0:
            subject_means.append(z[mask].mean(dim=0))

    subject_means = torch.stack(subject_means)
    global_mean = subject_means.mean(dim=0)
    mmd = torch.mean((subject_means - global_mean) ** 2)
    return mmd
...
def run_mmd(seed, lambda_mmd):
    ...
    for held_out in Y_SUBJECTS:
        ...
        for epoch in range(50):
            ...
            mmd = mmd_loss_simple(z, sub_tr_t, device)
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("EEG MMD Pilot Experiments (Optimized)")
    print("="*70)
    ...
    output_path = os.path.join(RESULTS_DIR, "eeg_mmd_pilot.csv")
