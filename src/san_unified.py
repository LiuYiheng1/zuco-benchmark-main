"""SAN: Source-Anchored Normalization - Unified Protocol"""
import os
...
print('SAN Experiments - Unified Protocol', flush=True)
print('='*60, flush=True)
...
for seed in seeds:
    ...
    for held_out in Y_SUBJECTS:
        ...
        for n_cal in shot_settings:
            ...
            X_cal_accs_san = normalize_X(X_cal_accs, y_cal_accs, mu_source_0, sigma_source_0, mu_source_1, sigma_source_1)
            X_test_san = normalize_X(X_test, y_test, mu_source_0, sigma_source_0, mu_source_1, sigma_source_1)
            acc, f1, bacc, auroc = train_and_evaluate(X_cal_accs_san, y_cal_accs, X_test_san, y_test)
            results.append({
                'seed': seed, 'subject': held_out, 'n_cal': n_cal,
                'method': 'SAN_ACCS', 'kappa': 0,
                'accuracy': acc, 'macro_f1': f1, 'balanced_accuracy': bacc, 'auroc': auroc
            })
    ...
    df.to_csv(RESULTS_DIR + '/san_results.csv', index=False)

print('', flush=True)
print('\n' + '='*60, flush=True)
print('SAN Results Summary', flush=True)
print('='*60, flush=True)
...
for n_cal in shot_settings:
    ...
    print(f'  SAN_ACCS: {san_accs:.4f} (gap={san_accs-accs:+.4f})', flush=True)

    for kappa in [10, 50, 100]:
        sasn = df[(df['method'] == 'SASN') & (df['n_cal'] == n_cal) & (df['kappa'] == kappa)]['accuracy'].mean()
        print(f'  SASN (kappa={kappa}): {sasn:.4f} (gap={sasn-baseline:+.4f})', flush=True)
