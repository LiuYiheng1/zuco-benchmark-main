"""SCI Framework - Simplified Standalone Version
...
print("="*70)
print("SCI Framework - Simplified Validation")
print("="*70)
...
def compute_domain_similarity(X_cal, y_cal, X_test, source_mu_0, source_mu_1, source_sigma_0, source_sigma_1):
    if len(np.unique(y_cal)) < 2:
        return np.ones(len(X_test)) * 0.5

    mu_0 = np.mean(X_cal[y_cal == 0], axis=0) if np.any(y_cal == 0) else source_mu_0
    mu_1 = np.mean(X_cal[y_cal == 1], axis=0) if np.any(y_cal == 1) else source_mu_1
    sigma_0 = np.std(X_cal[y_cal == 0], axis=0) + 1e-8 if np.any(y_cal == 0) else source_sigma_0
    sigma_1 = np.std(X_cal[y_cal == 1], axis=0) + 1e-8 if np.any(y_cal == 1) else source_sigma_1
...
print("\nRunning single-seed validation (seed=0, shots=[3,5,10,20,50])...")
...
for held_out in Y_SUBJECTS:
    ...
    source_mu_0 = np.mean(X_train_all[y_train_all == 0], axis=0) if np.any(y_train_all == 0) else np.mean(X_train_all, axis=0)
    source_mu_1 = np.mean(X_train_all[y_train_all == 1], axis=0) if np.any(y_train_all == 1) else np.mean(X_train_all, axis=0)
    source_sigma_0 = np.std(X_train_all[y_train_all == 0], axis=0) + 1e-8 if np.any(y_train_all == 0) else np.std(X_train_all, axis=0) + 1e-8
    source_sigma_1 = np.std(X_train_all[y_train_all == 1], axis=0) + 1e-8 if np.any(y_train_all == 1) else np.std(X_train_all, axis=0) + 1e-8
    ...
    for n_cal in shot_settings:
        ...
        mu_cal_0 = np.mean(X_cal[y_cal == 0], axis=0) if np.any(y_cal == 0) else source_mu_0
        mu_cal_1 = np.mean(X_cal[y_cal == 1], axis=0) if np.any(y_cal == 1) else source_mu_1
        sigma_cal_0 = np.std(X_cal[y_cal == 0], axis=0) + 1e-8 if np.any(y_cal == 0) else source_sigma_0
        sigma_cal_1 = np.std(X_cal[y_cal == 1], axis=0) + 1e-8 if np.any(y_cal == 1) else source_sigma_1
        ...
        p_domain = compute_domain_similarity(X_cal, y_cal, X_test, source_mu_0, source_mu_1, source_sigma_0, source_sigma_1)
        ...
        results.append({'seed': seed, 'subject': held_out, 'n_cal': n_cal, 'method': 'SIED', 'accuracy': acc_domain})
        ...
        for name, w_p, w_u, w_d in configs:
            ...
            results.append({'seed': seed, 'subject': held_out, 'n_cal': n_cal, 'method': name, 'accuracy': acc})
...
df.to_csv(f"{RESULTS_DIR}/sci_validation_results.csv", index=False)

print("\n\n" + "="*70)
print("Results Summary (Accuracy - Seed 0)")
print("="*70)
...
for n_cal in shot_settings:
    ...
    if best_sci_acc > 0:
        print(f"  {'SCI_best':15s}: {best_sci_acc:.4f} ({best_sci_name})", flush=True)

print("\n" + "="*70)
print("Key Finding: SCI vs Individual Modules")
print("="*70)
for n_cal in shot_settings:
    shot_df = df[df['n_cal'] == n_cal]
    ...
    marker = "✓ IMPROVEMENT" if improvement > 0 else ""
    print(f"{n_cal}-shot: Best SCI={best_sci_acc:.4f} vs Best Single={best_single:.4f} (Δ={improvement:+.4f}) [{best_sci_name}] {marker}")
