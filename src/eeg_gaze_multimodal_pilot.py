"""EEG-Gaze Multimodal Framework Pilot

Three innovations:
1. PCET: EEG Prediction-Error Representation
2. GETA: Gaze Entropy and Transition Attention
3. CAGF: Confidence-Aware Gated Fusion
...
def run_experiment():
    ...
    for seed in seeds:
        ...
        for held_out in Y_SUBJECTS:
            ...
            X_gaze_train_list, y_gaze_train_list = [], []
            ...
            X_eeg_train_all = np.vstack(X_eeg_train_list)
            y_eeg_train_all = np.concatenate(y_eeg_train_list)
            X_gaze_train_all = np.vstack(X_gaze_train_list)
            ...
            for n_cal in shot_settings:
                ...
                try:
                    ...
                    row['GETA_only_auroc'] = roc_auc_score(y_test_final, probs_geta)
                except:
                    row['GETA_only_acc'] = 0.5

                try:
                    p_pcet = np.clip(PCETModel().fit_predict(X_eeg_cal, y_cal, X_eeg_test_final).astype(float), 0, 1)
                    p_geta = np.clip(GETAModel().fit_predict(X_eeg_cal, y_cal, X_gaze_cal, X_eeg_test_final, X_gaze_test_final).astype(float), 0, 1)
                ...
                try:
                    ...
                    row['PCET+GETA+CAGF_auroc'] = roc_auc_score(y_test_final, probs_cagf)
                except:
...
def analyze_results(df):
    ...
    report.append("## Main Accuracy Comparison\n\n")
