"""SAN: Source-Anchored Normalization - CORRECTED (No double StandardScaler)"""
import os
...
def train_and_evaluate_no_scaler(X_cal, y_cal, X_test, y_test):
    """直接用已归一化的数据训练，不再做 StandardScaler"""
...
def train_and_evaluate_with_scaler(X_cal, y_cal, X_test, y_test):
    """使用 StandardScaler 的版本（用于 StandardScaler baseline）"""
    if len(np.unique(y_cal)) < 2:
        return 0.0, 0.0, 0.0, 0.5
    scaler = StandardScaler()
...
print('SAN Label-Free Experiments (CORRECTED)', flush=True)
print('='*60, flush=True)
...
for seed in seeds:
    ...
    for held_out in Y_SUBJECTS:
        ...
        for n_cal in shot_settings:
            ...
            acc, f1, bacc, auroc = train_and_evaluate_with_scaler(X_cal, y_cal, X_test, y_test)
            results.append({
            ...
            X_cal_global_san = normalize_X(X_cal, mu_source_global, sigma_source_global)
            X_test_global_san = normalize_X(X_test, mu_source_global, sigma_source_global)
            acc, f1, bacc, auroc = train_and_evaluate_no_scaler(X_cal_global_san, y_cal, X_test_global_san, y_test)
            results.append({
                'seed': seed, 'subject': held_out, 'n_cal': n_cal,
            ...
            X_cal_target_global = normalize_X(X_cal, mu_target_global, sigma_target_global)
            X_test_target_global = normalize_X(X_test, mu_target_global, sigma_target_global)
            acc, f1, bacc, auroc = train_and_evaluate_no_scaler(X_cal_target_global, y_cal, X_test_target_global, y_test)
            results.append({
                'seed': seed, 'subject': held_out, 'n_cal': n_cal,
            ...
            if len(np.unique(y_cal_accs)) < 2:
                continue
            acc, f1, bacc, auroc = train_and_evaluate_with_scaler(X_cal_accs, y_cal_accs, X_test, y_test)
            results.append({
                'seed': seed, 'subject': held_out, 'n_cal': n_cal,
            ...
            X_cal_global_san2 = normalize_X(X_cal_accs, mu_source_global, sigma_source_global)
            X_test_global_san2 = normalize_X(X_test, mu_source_global, sigma_source_global)
            acc, f1, bacc, auroc = train_and_evaluate_no_scaler(X_cal_global_san2, y_cal_accs, X_test_global_san2, y_test)
            results.append({
                'seed': seed, 'subject': held_out, 'n_cal': n_cal,
            ...
            if mu_cal_0 is not None and mu_cal_1 is not None:
                X_cal_class_norm = np.zeros_like(X_cal)
                for i in range(len(X_cal)):
                    if y_cal[i] == 0:
                        X_cal_class_norm[i] = (X_cal[i] - mu_cal_0) / sigma_cal_0
                    else:
                        X_cal_class_norm[i] = (X_cal[i] - mu_cal_1) / sigma_cal_1
                X_test_class_norm = np.zeros_like(X_test)
                for i in range(len(X_test)):
                    if y_test[i] == 0:
                        X_test_class_norm[i] = (X_test[i] - mu_cal_0) / sigma_cal_0
                    else:
                        X_test_class_norm[i] = (X_test[i] - mu_cal_1) / sigma_cal_1
                acc, f1, bacc, auroc = train_and_evaluate_no_scaler(X_cal_class_norm, y_cal, X_test_class_norm, y_test)
                results.append({
                    'seed': seed, 'subject': held_out, 'n_cal': n_cal,
                    'method': 'TargetNorm_labeled_cal', 'type': 'comparison',
                    'accuracy': acc, 'macro_f1': f1, 'balanced_accuracy': bacc, 'auroc': auroc
                })
            ...
            if mu_san_oracle_0 is not None and mu_san_oracle_1 is not None:
                ...
                acc, f1, bacc, auroc = train_and_evaluate_no_scaler(X_cal_oracle, y_cal, X_test_oracle, y_test)
                results.append({
                    'seed': seed, 'subject': held_out, 'n_cal': n_cal,
                    'method': 'SAN_oracle_class_cond', 'type': 'oracle',
                    'accuracy': acc, 'macro_f1': f1, 'balanced_accuracy': bacc, 'auroc': auroc
                })
    ...
    df.to_csv(RESULTS_DIR + '/san_label_free_results_v2.csv', index=False)

print('', flush=True)
print('\n' + '='*60, flush=True)
print('SAN Label-Free Results (CORRECTED)', flush=True)
print('='*60, flush=True)
...
for n_cal in shot_settings:
    ...
    target_labeled = df[(df['method'] == 'TargetNorm_labeled_cal') & (df['n_cal'] == n_cal)]['accuracy'].mean()
    san_oracle = df[(df['method'] == 'SAN_oracle_class_cond') & (df['n_cal'] == n_cal)]['accuracy'].mean()
