...
def flesch_reading_ease(text):
    ...
    if readability is not None:
        ...
        words = max(len(tokenized.split()), 1)
        sentences = max(text.count('.') + text.count('!') + text.count('?'), 1)
        syllables = sum(_estimate_syllables(word) for word in tokenized.split())
...
def _estimate_syllables(word):
    """Fallback syllable estimator used only when the readability package is unavailable."""
