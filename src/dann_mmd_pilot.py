"""
EEG DANN and MMD Pilot Experiments
"""
import os
import numpy as np
import pandas as pd
...
def run_dann(seed, lambda_adv):
    print(f"DANN lambda={lambda_adv} (seed={seed})...", flush=True)
    results = []
    ...
    for held_out in Y_SUBJECTS:
        ...
        results.append({
            'model': f'EEG_DANN_lamb{lambda_adv}',
            'seed': seed,
            'held_out': held_out,
            'accuracy': acc,
            'macro_f1': f1,
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("EEG DANN and MMD Pilot Experiments")
    print("="*70)

    print("\n--- DANN ---")
    for seed in [0, 1, 2]:
        for lambda_adv in [0.01, 0.05, 0.1]:
            all_results.extend(run_dann(seed, lambda_adv))
    ...
    output_path = os.path.join(RESULTS_DIR, "eeg_dann_mmd_pilot.csv")
