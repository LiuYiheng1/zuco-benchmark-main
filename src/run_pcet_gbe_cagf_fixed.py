...
def load_subject_data(subject):
    ...
    for key in eeg_feats.keys():
        ...
        if len(parts) >= 3:
            ...
            eeg_dict[sentence_idx] = {'key': key, 'data': eeg_feats[key], 'label': 0 if 'NR' in parts[1] else 1}
    ...
    for key in gaze_feats.keys():
        ...
        if len(parts) >= 3:
            sentence_idx = int(parts[2])
            gaze_dict[sentence_idx] = {'key': key, 'data': gaze_feats[key]}
...
for k in [3, 5, 10]:
    ...
    for seed in range(2):
        ...
        for subject in SUBJECTS[:4]:
            ...
            print(f"Processing {subject}: {len(y)} samples")
        ...
        if all_y_true:
            ...
            print(f"Seed {seed}: {len(all_y_true)} test samples, methods: {list(all_preds.keys())}")
