"""SCI: Staged Conditional Integration with Orthogonal Information Fusion

Key insight: Each module provides DIFFERENT type of information:
- PCET: Class prediction (what class)
- SRGC: Sample uncertainty (how confident is the model on this sample)
- SIED: Domain similarity (is this sample from source-like or target-like distribution)

These are ORTHOGONAL dimensions - combining them should improve over any single module.
"""
import os
...
def compute_gaussian_entropy(X, mu, sigma):
    """SRGC outputs uncertainty as Gaussian entropy"""
    sigma_diag = sigma ** 2 + 1e-8
    entropy = 0.5 * np.log(2 * np.pi * np.e * sigma_diag)
    return np.mean(entropy, axis=1)

def compute_mahalanobis(X, mu, sigma_inv):
    """Mahalanobis distance as uncertainty measure"""
    diff = X - mu
    mahal = np.sqrt(np.sum(np.dot(diff, sigma_inv) * diff, axis=1))
    return mahal
...
def train_sied_model(X_train, y_train, sub_ids, n_subjects, epochs=30, lambda_adv=0.01, dropout=0.3):
    ...
    with torch.no_grad():
        ...
        task_logits = task_clf(z)
        sub_logits = sub_disc(z)
        task_probs = torch.sigmoid(task_logits).cpu().numpy().flatten()
        sub_probs = torch.softmax(sub_logits, dim=1).cpu().numpy()

    subject_entropy = -np.sum(sub_probs * np.log(sub_probs + 1e-8), axis=1)
    mean_subject_entropy = np.mean(subject_entropy)

    return task_probs, mean_subject_entropy

print("="*70)
print("SCI: Staged Conditional Integration with Orthogonal Information Fusion")
print("="*70)
...
for seed in seeds:
    ...
    for held_out in Y_SUBJECTS:
        ...
        for n_cal in shot_settings:
            ...
            sigma_0_inv = np.linalg.inv(np.diag(sigma_cal_0 ** 2) + 1e-6)
            sigma_1_inv = np.linalg.inv(np.diag(sigma_cal_1 ** 2) + 1e-6)
            ...
            mahal_0_cal = compute_mahalanobis(X_cal_s, mu_cal_0, sigma_0_inv)
            mahal_1_cal = compute_mahalanobis(X_cal_s, mu_cal_1, sigma_1_inv)
            mahal_0_test = compute_mahalanobis(X_test_s, mu_cal_0, sigma_0_inv)
            mahal_1_test = compute_mahalanobis(X_test_s, mu_cal_1, sigma_1_inv)

            uncertainty_cal = np.abs(mahal_1_cal - mahal_0_cal)
            uncertainty_test = np.abs(mahal_1_test - mahal_0_test)
            ...
            c_pcet_cal = np.maximum(p_pcet_cal, 1 - p_pcet_cal)
            c_pcet_test = np.maximum(p_pcet_test, 1 - p_pcet_test)
            ...
            try:
                sied_probs_cal, sied_entropy_cal = train_sied_model(X_cal, y_cal, sub_ids_all[:len(y_cal)], n_subjects, epochs=30, lambda_adv=0.01)
                sied_probs_test, sied_entropy_test = train_sied_model(X_test, y_test, np.zeros(len(y_test)), n_subjects, epochs=30, lambda_adv=0.01)
            except:
                sied_probs_cal = np.ones(len(y_cal)) * 0.5
                sied_probs_test = np.ones(len(y_test)) * 0.5
                sied_entropy_cal = 0.5
                sied_entropy_test = 0.5
            ...
            y_sied_pred = (sied_probs_test >= 0.5).astype(int)
            ...
            try:
                ...
                auroc_sied = roc_auc_score(y_test, sied_probs_test)
            except:
                auroc_eeg = auroc_pcet = auroc_srgc = auroc_sied = 0.5

            results.append({'seed': seed, 'subject': held_out, 'n_cal': n_cal, 'method': 'EEG_SVM', 'accuracy': acc_eeg, 'macro_f1': f1_eeg, 'balanced_accuracy': bacc_eeg, 'auroc': auroc_eeg})
            ...
            results.append({'seed': seed, 'subject': held_out, 'n_cal': n_cal, 'method': 'SIED', 'accuracy': acc_sied, 'macro_f1': f1_sied, 'balanced_accuracy': bacc_sied, 'auroc': auroc_sied})

            for w_pcet in [0.5, 0.6, 0.7]:
                for w_unc in [0.1, 0.2, 0.3]:
                    for w_dom in [0.1, 0.2]:
                        w_sied = 1 - w_pcet - w_unc - w_dom
                        if w_sied < 0 or w_sied > 0.5:
                            continue

                        p_unc = 1 - uncertainty_test / (uncertainty_test.max() + 1e-8)
                        p_dom = 1 - np.abs(sied_probs_test - 0.5) * 2

                        p_ortho = w_pcet * p_pcet_test + w_unc * p_unc + w_dom * p_dom
                        y_ortho_pred = (p_ortho >= 0.5).astype(int)
                        acc_ortho = accuracy_score(y_test, y_ortho_pred)
                        f1_ortho = f1_score(y_test, y_ortho_pred, average='macro')
                        bacc_ortho = balanced_accuracy_score(y_test, y_ortho_pred)
                        try:
                            auroc_ortho = roc_auc_score(y_test, p_ortho)
                        except:
                            auroc_ortho = 0.5

                        results.append({
                            'seed': seed, 'subject': held_out, 'n_cal': n_cal,
                            'method': f'ORTHO_w{w_pcet:.1f}{w_unc:.1f}{w_dom:.1f}',
                            'accuracy': acc_ortho, 'macro_f1': f1_ortho,
                            'balanced_accuracy': bacc_ortho, 'auroc': auroc_ortho
                        })

            for alpha_pcet in [0.5, 0.6, 0.7, 0.8]:
                for alpha_srgc in [0.1, 0.2, 0.3]:
                    alpha_sied = 1 - alpha_pcet - alpha_srgc
                    if alpha_sied < 0 or alpha_sied > 0.4:
                        continue

                    p_staged = alpha_pcet * p_pcet_test + alpha_srgc * p_srgc_test + alpha_sied * sied_probs_test
                    y_staged_pred = (p_staged >= 0.5).astype(int)
                    acc_staged = accuracy_score(y_test, y_staged_pred)
                    f1_staged = f1_score(y_test, y_staged_pred, average='macro')
                    bacc_staged = balanced_accuracy_score(y_test, y_staged_pred)
                    try:
                        auroc_staged = roc_auc_score(y_test, p_staged)
                    except:
                        auroc_staged = 0.5

                    results.append({
                        'seed': seed, 'subject': held_out, 'n_cal': n_cal,
                        'method': f'STAGED_a{alpha_pcet:.1f}{alpha_srgc:.1f}{alpha_sied:.1f}',
                        'accuracy': acc_staged, 'macro_f1': f1_staged,
                        'balanced_accuracy': bacc_staged, 'auroc': auroc_staged
                    })

        print(f'.', end='', flush=True)

    df_partial = pd.DataFrame(results)
    df_partial.to_csv(f"{RESULTS_DIR}/sci_partial.csv", index=False)

df = pd.DataFrame(results)
df.to_csv(f"{RESULTS_DIR}/sci_results.csv", index=False)
...
for n_cal in shot_settings:
    ...
    ortho_methods = shot_df[shot_df['method'].str.startswith('ORTHO')]['method'].unique()
    staged_methods = shot_df[shot_df['method'].str.startswith('STAGED')]['method'].unique()

    if len(ortho_methods) > 0:
        best_ortho_acc = 0
        best_ortho_method = ''
        for m in ortho_methods:
            m_df = shot_df[shot_df['method'] == m]
            acc = m_df['accuracy'].mean()
            if acc > best_ortho_acc:
                best_ortho_acc = acc
                best_ortho_method = m
        print(f"  {'ORTHO_best':15s}: {best_ortho_acc:.4f} ({best_ortho_method})", flush=True)

    if len(staged_methods) > 0:
        best_staged_acc = 0
        best_staged_method = ''
        for m in staged_methods:
            m_df = shot_df[shot_df['method'] == m]
            acc = m_df['accuracy'].mean()
            if acc > best_staged_acc:
                best_staged_acc = acc
                best_staged_method = m
        print(f"  {'STAGED_best':15s}: {best_staged_acc:.4f} ({best_staged_method})", flush=True)
