"""SIED Lambda Sensitivity Analysis"""
import os
...
def run_sied_with_lambda(seed, lambda_adv):
    print(f"  lambda={lambda_adv} seed={seed}", flush=True)
    results = []
...
def main():
    print("="*60)
    print("SIED Lambda Sensitivity Analysis")
    print("="*60)

    all_results = []
    seeds = [0, 1, 2, 3, 4]
    lambda_values = [0, 0.001, 0.005, 0.01, 0.05, 0.1]

    for seed in seeds:
        print(f"\nSeed {seed}:", flush=True)
        for lambda_adv in lambda_values:
            all_results.extend(run_sied_with_lambda(seed, lambda_adv))

    df = pd.DataFrame(all_results)
    df.to_csv("results/final/sied_lambda_sensitivity.csv", index=False)

    print("\n" + "="*60)
    print("Results Summary by Lambda")
    print("="*60)

    for lambda_adv in lambda_values:
        data = df[df['model'] == f'SIED_l{lambda_adv}']
        if len(data) > 0:
            ...
            print(f"  lambda={lambda_adv:6.3f}: acc={acc:.4f}±{std:.4f}, f1={f1:.4f}, bacc={bacc:.4f}, sub_pred={sub_pred:.4f}")

    print("\n" + "="*60)
    print("Trade-off Analysis")
    print("="*60)

    baseline_acc = df[df['model'] == 'SIED_l0']['accuracy'].mean()
    print(f"\nBaseline (lambda=0) task accuracy: {baseline_acc:.4f}")
    ...
    for lambda_adv in lambda_values:
        ...
        print(f"  lambda={lambda_adv:6.3f}: task_acc={acc:.4f} (gap={acc-baseline_acc:+.4f}), sub_pred={sub_pred:.4f} (vs random={random_baseline:.4f})")
