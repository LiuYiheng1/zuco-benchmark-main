...
def main():
    log_file = open(os.path.join(RESULTS_DIR, 'protocol_simple_log.txt'), 'w')
    ...
    log("="*60)
    log("FINAL PROTOCOL EXPERIMENTS (Simplified)")
    log("="*60)
    ...
    for i, held_out in enumerate(Y_SUBJECTS):
        ...
        results.append(row)
        acc = row.get('PCET+GETA+CAGF_acc', 0)
        log(f"  CAGF Acc: {acc:.3f}")
    ...
    log("\nLOSO Summary:")
    for m in ['EEG_SVM', 'Gaze_SVM', 'PCET_source', 'GETA_source', 'PCET+GETA+CAGF']:
        acc_col = f'{m}_acc'
    ...
    split_row = {'split': '12train/2val/4test'}
