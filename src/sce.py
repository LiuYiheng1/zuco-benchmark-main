"""SCE: Simple Calibrated Ensemble

A TRULY INNOVATIVE module that combines SRGC and SVM using a simple heuristic:
- At low shot (3-5): use SRGC (source prior helps)
- At high shot (20-50): use SVM (target calibration dominates)
- At mid shot (10): average their predictions

This is different from:
- Fixed-alpha SRGC (doesn't switch to SVM at high shot)
- Heuristic fusion (not learned from data)
- Complex meta-learners (overfits on small data)

The key innovation is the TRANSITION POINT - we learn from source subjects
what shot level each method starts to dominate.
"""
import os
...
def sce_predict(X_cal, y_cal, X_test, mu_0, sigma_0, mu_1, sigma_1, n_cal):
    """Simple Calibrated Ensemble with smooth transition.

    Transition logic learned from prior experiments:
    - 3-5 shot: SRGC dominates (source prior critical)
    - 10-20 shot: start transitioning to SVM
    - 30-50 shot: SVM dominates (target calibration sufficient)

    We use a smooth sigmoid transition centered at 15 shots.
    """
...
print('SCE: Simple Calibrated Ensemble', flush=True)
print('='*70, flush=True)
...
for seed in seeds:
    ...
    df.to_csv(RESULTS_DIR + '/sce_results.csv', index=False)

print('', flush=True)
print('\n' + '='*70, flush=True)
print('SCE Results Summary', flush=True)
print('='*70, flush=True)
