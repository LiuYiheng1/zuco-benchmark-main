...
class CIRFNet(nn.Module):
    def __init__(self, text_dim, eeg_dim, gaze_dim, 
                proj_dim=64, hidden_dim=128, 
                use_text_modulation=True, use_agreement=True, use_conflict=True,
                lambda_inter=0.5):
        super().__init__()
        
        self.use_text_modulation = use_text_modulation
        ...
        self.backbone_head = nn.Linear(hidden_dim // 2, 2)
        ...
        # Text-conditioned modulation (FiLM)
        if self.use_text_modulation:
            self.film_e = nn.Sequential(
                nn.Linear(proj_dim, proj_dim),
                nn.ReLU(),
                nn.Linear(proj_dim, proj_dim * 2)
            )
            self.film_g = nn.Sequential(
                nn.Linear(proj_dim, proj_dim),
                nn.ReLU(),
                nn.Linear(proj_dim, proj_dim * 2)
            )
        ...
        # Interaction MLP
        inter_input_dim = proj_dim * 3  # z_t, z_e_tilde, z_g_tilde
        ...
        self.inter_mlp = nn.Sequential(
            nn.Linear(inter_input_dim, hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(hidden_dim // 2, hidden_dim // 4),
            nn.ReLU(),
        )
        self.inter_head = nn.Linear(hidden_dim // 4, 2)
    ...
    def forward(self, x_text, x_eeg, x_gaze):
        ...
        if self.use_text_modulation:
            film_e_out = self.film_e(z_t)
            gamma_e = film_e_out[:, :self.film_e[-1].out_features//2]
            beta_e = film_e_out[:, self.film_e[-1].out_features//2:]
            z_e_tilde = gamma_e * z_e + beta_e
            
            film_g_out = self.film_g(z_t)
            gamma_g = film_g_out[:, :self.film_g[-1].out_features//2]
            beta_g = film_g_out[:, self.film_g[-1].out_features//2:]
            z_g_tilde = gamma_g * z_g + beta_g
        else:
            z_e_tilde = z_e
            z_g_tilde = z_g
        
        # Interaction features
        inter_features = [z_t, z_e_tilde, z_g_tilde]
        if self.use_agreement:
            inter_features.append(z_e_tilde * z_g_tilde)
        if self.use_conflict:
            inter_features.append(torch.abs(z_e_tilde - z_g_tilde))
        
        h_inter = torch.cat(inter_features, dim=1)
        h_inter = self.inter_mlp(h_inter)
        logit_inter = self.inter_head(h_inter)
        
        # Residual fusion
        logit_final = logit_base + self.lambda_inter * logit_inter
        
        return logit_final, logit_base, logit_inter


def train_model(model, X_text_train, X_eeg_train, X_gaze_train, y_train, 
                X_text_val, X_eeg_val, X_gaze_val, y_val,
                n_epochs=50, lr=0.001, device='cpu'):
...
def run_experiment():
    ...
    for seed in range(3):
        ...
        # CIRF-Net full
        model = CIRFNet(text_dim, eeg_dim, gaze_dim, 
                        use_text_modulation=True, use_agreement=True, use_conflict=True)
        result = train_model(model, X_text_train, X_eeg_train_s, X_gaze_train_s, y_train,
                            X_text_test, X_eeg_test_s, X_gaze_test_s, y_test,
                            device=device)
        result['method'] = 'CIRF-Net_full'
        result['seed'] = seed
        results.append(result)
        print(f"CIRF-Net_full: {result['accuracy']:.4f}")
        
        # CIRF-Net w/o text modulation
        model = CIRFNet(text_dim, eeg_dim, gaze_dim,
                        use_text_modulation=False, use_agreement=True, use_conflict=True)
        result = train_model(model, X_text_train, X_eeg_train_s, X_gaze_train_s, y_train,
                            X_text_test, X_eeg_test_s, X_gaze_test_s, y_test,
                            device=device)
        result['method'] = 'CIRF-Net_w/o_modulation'
        result['seed'] = seed
        results.append(result)
        print(f"CIRF-Net_w/o_modulation: {result['accuracy']:.4f}")
        
        # CIRF-Net w/o agreement
        model = CIRFNet(text_dim, eeg_dim, gaze_dim,
                        use_text_modulation=True, use_agreement=False, use_conflict=True)
        result = train_model(model, X_text_train, X_eeg_train_s, X_gaze_train_s, y_train,
                            X_text_test, X_eeg_test_s, X_gaze_test_s, y_test,
                            device=device)
        result['method'] = 'CIRF-Net_w/o_agreement'
        result['seed'] = seed
        results.append(result)
        print(f"CIRF-Net_w/o_agreement: {result['accuracy']:.4f}")
        
        # CIRF-Net w/o conflict
        model = CIRFNet(text_dim, eeg_dim, gaze_dim,
                        use_text_modulation=True, use_agreement=True, use_conflict=False)
        result = train_model(model, X_text_train, X_eeg_train_s, X_gaze_train_s, y_train,
                            X_text_test, X_eeg_test_s, X_gaze_test_s, y_test,
                            device=device)
        result['method'] = 'CIRF-Net_w/o_conflict'
        result['seed'] = seed
        results.append(result)
        print(f"CIRF-Net_w/o_conflict: {result['accuracy']:.4f}")
...
if __name__ == '__main__':
    print("=" * 80)
    print("CIRF-Net: Concat-preserving Interaction Residual Fusion Network")
    print("=" * 80)
    ...
    df.to_csv('results/final/cirf_net_results.csv', index=False)
    
    print("\n\nFiles saved:")
    print("  - results/final/cirf_net_results.csv")
    
    print("\n" + "=" * 80)
    print("CIRF-Net Experiment Complete")
    print("=" * 80)