"""
SIED + Component-wise SupCon Experiments
...
Models:
1. Raw_EEG - baseline
2. SIED - subject-invariant adversarial
3. SIED_TaskSupCon - SIED + supervised contrastive on task embeddings
4. SIED_DomainSupCon - SIED + domain separation
5. SIED_ComponentSupCon - SIED + component-wise supervised contrastive
...
def domain_separation_loss(z, sub_ids, temperature=0.1):
    """Push apart same-subject embeddings (domain separation)"""
    device = z.device
    batch_size = z.shape[0]
    ...
    same_subj_mask = torch.eq(sub_ids.unsqueeze(0), sub_ids.unsqueeze(1)).float().to(device)
    mask_no_self = same_subj_mask - torch.eye(batch_size).to(device)

    pos_sim = (sim_matrix * mask_no_self).sum(dim=1) / (mask_no_self.sum(dim=1) + 1e-8)

    loss = torch.exp(pos_sim).mean()

    return loss

def compute_correlation(z1, z2):
    """Compute correlation between two embedding sets"""
    z1 = z1.detach().cpu().numpy()
    z2 = z2.detach().cpu().numpy()

    corrs = []
    for i in range(z1.shape[1]):
        r, _ = pearsonr(z1[:, i], z2[:, i])
        if not np.isnan(r):
            corrs.append(abs(r))
...
def run_sied(seed, lambda_adv=1.0):
    print(f"  SIED lambda={lambda_adv} (seed={seed})...")
    results = []
...
def run_sied_task_supcon(seed, lambda_adv=1.0, beta=0.1, temperature=0.1):
    print(f"  SIED_TaskSupCon beta={beta} temp={temperature} (seed={seed})...")
    results = []
...
def run_sied_domain_supcon(seed, lambda_adv=1.0, beta=0.1, temperature=0.1):
    print(f"  SIED_DomainSupCon beta={beta} temp={temperature} (seed={seed})...")
    results = []
    ...
    for held_out in Y_SUBJECTS:
        try:
            ...
            for epoch in range(50):
                ...
                domain_loss = domain_separation_loss(z, sub_tr_t, temperature=temperature)

                loss = task_loss + lambda_adv * sub_loss + beta * domain_loss
            ...
            results.append({
                'model': f'SIED_DomainSupCon_b{beta}_t{temperature}', 'seed': seed, 'held_out': held_out,
                'accuracy': acc, 'macro_f1': f1, 'balanced_accuracy': bacc, 'auroc': auroc
            })
...
def main():
    ...
    print("="*60)
    print("SIED + Component-wise SupCon Experiments")
    print("="*60)
    ...
    for seed in seeds[:3]:
        for beta in beta_values:
            for temp in temp_values:
                all_results.extend(run_sied_domain_supcon(seed, lambda_adv=1.0, beta=beta, temperature=temp))
    ...
    supcon_models = [m for m in df['model'].unique() if 'TaskSupCon' in m or 'DomainSupCon' in m]

    print("\nMain models:")
    for model in ['Raw_EEG', 'SIED_l1.0']:
        data = df[df['model'] == model]
        if len(data) > 0:
            acc = data['accuracy'].mean()
            std = data['accuracy'].std()
            f1 = data['macro_f1'].mean()
            print(f"  {model:40s}: acc={acc:.4f}±{std:.4f}, f1={f1:.4f}")

    print("\nTop 10 SIED_TaskSupCon:")
    for model in sorted(supcon_models):
        if 'TaskSupCon' in model:
            data = df[df['model'] == model]
            if len(data) > 0:
                acc = data['accuracy'].mean()
                std = data['accuracy'].std()
    ...
    print("\nTop 10 SIED_DomainSupCon:")
    for model in sorted(supcon_models):
        if 'DomainSupCon' in model:
            data = df[df['model'] == model]
            if len(data) > 0:
                acc = data['accuracy'].mean()
                std = data['accuracy'].std()
    ...
    print("="*60)
    print(f"Target: exceed SIED +1.5% = {sied_baseline + 0.015:.4f}")
    ...
    if best_models:
        ...
        for model, acc in sorted(best_models, key=lambda x: -x[1])[:10]:
