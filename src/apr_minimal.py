...
for seed in [0,1,2]:
    ...
    np.random.seed(seed)
    for ho in Y_SUBJECTS[:8]:
        Xt, yt = cache[ho]
        if Xt is None: continue
        Xtr, ytr = [], []
        for s in Y_SUBJECTS[:8]:
        ...
        for n_cal in [3,5,10,20,50]:
            ...
            for m, p in [('SVM', svm_pred(Xc, yc, X_test)),
                          ('SRGC', fixed_pred(Xc, yc, X_test, ms0, ss0, ms1, ss1))]:
                results.append({'seed':seed,'subject':ho,'n_cal':n_cal,'method':m,'k':np.nan,'nu':np.nan,'acc':accuracy_score(y_test, p)})
            for k, nu in kappa_nu_list:
                p = apr_pred(Xc, yc, X_test, ms0, ss0, ms1, ss1, k, nu)
                results.append({'seed':seed,'subject':ho,'n_cal':n_cal,'method':'APR','k':k,'nu':nu,'acc':accuracy_score(y_test, p)})
...
for n in [3,5,10,20,50]:
    ...
    for m in ['SVM', 'SRGC']:
        ...
        if len(d) > 0: print(f'  {m}: {d.acc.mean():.4f}')
    apr = df[(df.method=='APR') & (df.n_cal==n)]
    if len(apr) > 0:
        best = apr.groupby(['k','nu'])['acc'].mean().idxmax()
        best_acc = apr.groupby(['k','nu'])['acc'].mean().max()
        print(f'  APR best: {best_acc:.4f} (k={best[0]}, nu={best[1]})')
print('Done!')