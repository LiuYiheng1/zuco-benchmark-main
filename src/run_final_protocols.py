"""Final Protocol Experiments for Paper - Simplified Version"""
...
def run_experiment(all_data, split_name="LOSO"):
    results = []
    if split_name == "LOSO":
        subject_list = Y_SUBJECTS
    else:
        subject_list = Y_SUBJECTS

    for held_out in subject_list:
        train_subjs = [s for s in subject_list if s != held_out and s in all_data]
        test_subj = held_out
        ...
        results.append(row)
        print(f"[{held_out}] done", flush=True)

    return pd.DataFrame(results)

def run_split_experiment(all_data, train_subjs, val_subjs, test_subjs):
    X_eeg_train = np.vstack([all_data[s]['Xe'] for s in train_subjs])
    y_train = np.concatenate([all_data[s]['ye'] for s in train_subjs])
    X_gaze_train = np.vstack([all_data[s]['Xg'] for s in train_subjs])
    ...
    print(f"[Split] done")
    return pd.DataFrame([row]), train_subjs, val_subjs, test_subjs
...
def main():
    ...
    print("\n" + "="*60)
    print("PROTOCOL 1: BENCHMARK-STYLE LOSO")
    print("="*60)
    df_loso = run_experiment(all_data, "LOSO")
    ...
    summary_data = {'metric': ['acc_mean', 'acc_std', 'f1_mean', 'f1_std', 'bacc_mean', 'bacc_std', 'auroc_mean', 'auroc_std']}
    for m in methods:
        acc_col = f'{m}_acc' if m not in ['Majority', 'Random'] else m
        if acc_col in df_loso.columns:
            summary_data[m] = [
                f"{df_loso[acc_col].mean()*100:.1f}",
                f"{df_loso[acc_col].std()*100:.1f}",
                f"{df_loso[f'{m}_f1'].mean()*100:.1f}" if f'{m}_f1' in df_loso.columns else "N/A",
                f"{df_loso[f'{m}_f1'].std()*100:.1f}" if f'{m}_f1' in df_loso.columns else "N/A",
                f"{df_loso[f'{m}_bacc'].mean()*100:.1f}" if f'{m}_bacc' in df_loso.columns else "N/A",
                f"{df_loso[f'{m}_bacc'].std()*100:.1f}" if f'{m}_bacc' in df_loso.columns else "N/A",
                f"{df_loso[f'{m}_auroc'].mean()*100:.1f}" if f'{m}_auroc' in df_loso.columns else "N/A",
                f"{df_loso[f'{m}_auroc'].std()*100:.1f}" if f'{m}_auroc' in df_loso.columns else "N/A",
            ]
    ...
    df_split, _, _, _ = run_split_experiment(all_data, train_subjs, val_subjs, test_subjs)
    ...
    loso_acc = df_loso['PCET+GETA+CAGF_acc'].mean() * 100 if 'PCET+GETA+CAGF_acc' in df_loso.columns else 0
    loso_f1 = df_loso['PCET+GETA+CAGF_f1'].mean() * 100 if 'PCET+GETA+CAGF_f1' in df_loso.columns else 0
    loso_std = df_loso['PCET+GETA+CAGF_acc'].std() * 100 if 'PCET+GETA+CAGF_acc' in df_loso.columns else 0
    split_acc = df_split['PCET+GETA+CAGF_acc'].values[0] * 100 if 'PCET+GETA+CAGF_acc' in df_split.columns else 0
    split_f1 = df_split['PCET+GETA+CAGF_f1'].values[0] * 100 if 'PCET+GETA+CAGF_f1' in df_split.columns else 0
...
"""
    ...
    print(f"\nKey Questions:")
    print(f"1. Benchmark-style LOSO: {loso_acc:.1f}%")
    print(f"2. AdaGTCN-style 12/2/4: {split_acc:.1f}%")
    print(f"3. vs ZuCo Benchmark eye-tracking (69%): {'YES' if loso_acc > 69 else 'NO'}")
    print(f"4. vs AdaGTCN reported (69.79%): {'YES' if loso_acc > 69.79 else 'NO'}")
    print("\nDone!")
