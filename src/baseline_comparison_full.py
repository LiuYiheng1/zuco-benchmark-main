"""Comprehensive Baseline Comparison for Personalized Few-Shot Setting

Compares:
1. Basic baselines: Majority, Random, LogisticRegression, LinearSVM, RBF_SVM, LDA, QDA, GaussianNB, KNN, RandomForest
2. Neural baselines: EEG_MLP, MLP_Dropout, Residual_MLP
3. Few-shot/Gaussian baselines: NearestClassMean, Prototype_TSPC, TargetOnly_Gaussian, SourceOnly_Gaussian, SRGC
4. Ours and combinations: SIED, PCET, SRGC, PCET_SRGC, SIED_PCET, SIED_SRGC, SIED_PCET_SRGC
"""
import os
...
all_methods = [
    ...
    'LDA', 'QDA', 'GaussianNB', 'KNN', 'RandomForest',
    'EEG_MLP', 'MLP_Dropout', 'Residual_MLP',
    'NearestClassMean', 'Prototype_TSPC', 'TargetOnly_Gaussian', 'SourceOnly_Gaussian',
    'SIED', 'PCET', 'SRGC',
    'PCET_SRGC', 'SIED_PCET', 'SIED_SRGC', 'SIED_PCET_SRGC'
]
            ...
            for method in all_methods:
                try:
                    if method == 'Majority':
                        ...
                        clf.fit(X_cal_s, y_cal)
                        y_pred = clf.predict(X_test_s)
                    elif method == 'MLP_Dropout':
                        scaler = StandardScaler()
                        X_cal_s = scaler.fit_transform(X_cal)
                        X_test_s = scaler.transform(X_test)
                        clf = MLPClassifier(hidden_layer_sizes=(128, 64), dropout=0.3, max_iter=500, random_state=seed)
                        clf.fit(X_cal_s, y_cal)
                        y_pred = clf.predict(X_test_s)
                        ...
                        X_cal_s = scaler.fit_transform(X_cal)
                        X_test_s = scaler.transform(X_test)
                        clf = MLPClassifier(hidden_layer_sizes=(128, 64, 128), max_iter=500, random_state=seed)
                        clf.fit(X_cal_s, y_cal)
                        y_pred = clf.predict(X_test_s)
