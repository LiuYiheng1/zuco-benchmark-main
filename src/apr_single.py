...
def apr_pred(Xc, yc, Xt, ms0, ss0, ms1, ss1, k, nu):
    ...
    l0 = k/(k+n0) if n0 > 0 else 0.5
    l1 = k/(k+n1) if n1 > 0 else 0.5
    g0 = nu/(nu+n0) if n0 > 0 else 0.5
    g1 = nu/(nu+n1) if n1 > 0 else 0.5
...
results = []
kappa_nu_list = [(1,1), (5,5), (10,10), (20,20), (50,50), (1,50), (5,10)]

for seed in [0]:
    np.random.seed(seed)
    for held_out in ['YAC', 'YAG']:
        Xt, yt = cache[held_out]
        Xtr, ytr = [], []
        for s in Y_SUBJECTS:
            if s != held_out:
                X, y = cache[s]
        ...
        ms0, ss0 = stats(Xtr, ytr, 0)
        ms1, ss1 = stats(Xtr, ytr, 1)
        ...
        print(f'Held-out: {held_out}, Test: {len(y_test)}, Pool: {len(ycp)}')
        ...
        for n_cal in [3, 5, 10, 20, 50]:
            ...
            ci = sample(ycp, n_cal)
            Xc, yc = Xcp[ci], ycp[ci]
            print(f'  {n_cal}-shot: cal samples={len(yc)}, class dist={np.bincount(yc)}')

            if len(np.unique(yc)) < 2:
                continue

            svm_p = svm_pred(Xc, yc, X_test)
            results.append({'n_cal': n_cal, 'method': 'SVM', 'acc': accuracy_score(y_test, svm_p)})

            fixed_p = fixed_pred(Xc, yc, X_test, ms0, ss0, ms1, ss1)
            results.append({'n_cal': n_cal, 'method': 'SRGC', 'acc': accuracy_score(y_test, fixed_p)})

            for k, nu in kappa_nu_list:
                apr_p = apr_pred(Xc, yc, X_test, ms0, ss0, ms1, ss1, k, nu)
                results.append({'n_cal': n_cal, 'method': f'APR_k{k}_nu{nu}', 'acc': accuracy_score(y_test, apr_p)})

df = pd.DataFrame(results)
print('\n' + '='*50)
print('RESULTS (Subject YAC, seed=0):')
print('='*50)
for n in [3, 5, 10, 20, 50]:
    print(f'\n{n}-shot:')
    d = df[df.n_cal == n]
    for m in ['SVM', 'SRGC'] + [f'APR_k{k}_nu{nu}' for k, nu in kappa_nu_list]:
        row = d[d.method == m]
        if len(row) > 0:
