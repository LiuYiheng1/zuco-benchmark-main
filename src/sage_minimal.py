...
print('SAGE Minimal Test', flush=True)
...
def sr_pred(Xc, yc, Xt, ms0, ss0, ms1, ss1, a=0.25):
    ...
    if mc0 is None: return np.zeros(len(Xt)), np.zeros(len(Xt)), np.zeros(len(Xt))
...
cache = {s: load_eeg(s) for s in Y_SUBJECTS[:8]}
...
for seed in [0]:
    ...
    for ho in Y_SUBJECTS[:4]:
        ...
        for n_cal in [3,5,10,20,50]:
            ...
            results.append({'seed':seed,'subject':ho,'n_cal':n_cal,'method':'SAGE_rule','lambda':lam,'accuracy':accuracy_score(y_test, preds_fused)})
        print('.', end='', flush=True)
    print('')

df = pd.DataFrame(results)
os.makedirs('results/final', exist_ok=True)
df.to_csv('results/final/sage_results_minimal.csv', index=False)
print('Saved!', flush=True)

print('\nResults:')
for n in [3,5,10,20,50]:
    for m in ['LR', 'SR-GC', 'SAGE_rule']:
        subset = df[(df.method==m) & (df.n_cal==n)]
        if len(subset) > 0:
            print(f'  {n}-shot {m}: {subset.accuracy.mean():.4f}')
print('Done!')