"""
SCI Framework V4: Simplified Confidence-Based Fusion

Design:
1. PCET provides class prediction
2. SRGC provides uncertainty (0=confident, 1=uncertain)
3. SIED provides domain score (0=in-dist, 1=ood)

Simple fusion:
- If PCET confident (conf > tau) → use PCET
- If PCET uncertain (conf < tau) and SRGC confident → use SRGC
- Domain shift adjusts decision threshold
"""
import os
import numpy as np
...
print("="*60)
print("SCI Framework V4: Simplified")
print("="*60)
...
class SRGCUncertainty:
    """SRGC: Estimates uncertainty via distance-based method"""
    ...
    def predict_uncertainty(self, X_test):
        """Return uncertainty: 0=confident, 1=uncertain"""
        X_test_s = self.scaler.transform(X_test)
...
class SIEDDomain:
    """SIED: Detects domain shift via feature distribution comparison"""
    ...
    def predict_domain_shift(self, X_test):
        """Return domain shift: 0=in-distribution, 1=ood"""
        X_test_s = self.scaler.transform(X_test)

        test_mean = np.mean(X_test_s, axis=0)
        mean_diff = np.sqrt(np.sum((test_mean - self.mu_cal) ** 2))

        feat_std = np.std(X_test_s, axis=0)
        std_ratio = np.mean(feat_std / self.sigma_cal)

        shift = 0.7 * mean_diff / (np.sqrt(np.sum(self.mu_cal ** 2)) + 1e-8) + 0.3 * np.abs(std_ratio - 1)
        return np.ones(len(X_test)) * np.clip(shift, 0, 1)


def simple_fusion(p_pcet, conf_pcet, uncertainty_srgc, domain_shift,
                   tau_conf=0.5, tau_unc=0.3, tau_dom=0.5):
    """
    Simple fusion logic:
    1. If PCET confident → use PCET
    2. If PCET uncertain but SRGC confident → blend
    3. Domain shift raises threshold (be more conservative)
    """
    ...
    for i in range(len(p_pcet)):
        p = p_pcet[i]
        conf = conf_pcet[i]
        unc = uncertainty_srgc[i]
        dom = domain_shift[i]

        if dom > tau_dom:
            threshold = 0.5 + 0.1 * dom
        else:
            threshold = 0.5

        if conf > tau_conf:
            preds[i] = 1 if p >= threshold else 0
        elif unc < tau_unc:
            blend = 0.7 * p + 0.3 * (0.5 if unc > 0.5 else 0.8)
            preds[i] = 1 if blend >= threshold else 0
        else:
            preds[i] = 1 if p >= threshold else 0
...
for seed in seeds:
    ...
    for held_out in Y_SUBJECTS:
        ...
        for n_cal in shots:
            ...
            for tau_c in [0.4, 0.5, 0.6]:
                for tau_u in [0.2, 0.3, 0.4]:
                    for tau_d in [0.3, 0.5, 0.7]:
                        y_fusion = simple_fusion(p_pcet, conf_pcet, uncertainty, domain_shift,
                                               tau_conf=tau_c, tau_unc=tau_u, tau_dom=tau_d)
                        acc_fusion = accuracy_score(y_test, y_fusion)
                        results.append({
                            'seed': seed, 'subject': held_out, 'n_cal': n_cal,
                            'method': f'F_c{tau_c}_u{tau_u}_d{tau_d}',
                            'accuracy': acc_fusion
                        })
...
df.to_csv(f'{RESULTS_DIR}/sci_v4_results.csv', index=False)
